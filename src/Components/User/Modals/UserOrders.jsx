...
import { My_Orders, View_MyOrders } from '../../../utils/Constants';
...
function UserOrders() {
  ...
  const getOrderedPosts = async () => {
    try {
      const response = await axios.get(`${My_Orders}${userId}`)
      console.log(response.data.data,'responsssssse');
        ...
        <Card variant="outlined" sx={{my:5 , mx:2}}>
          <CardContent>
            ...
            <Typography variant="body2">
              <span style={{ fontWeight: 'bold'}}>Buyer : </span>{orderedpost.user_name}
              <br />
